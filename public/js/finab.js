...
function createTableFragment() {
    ...
    .then(data => {
        const datafinab = data.sort();
        const finabData = datafinab;
        const thead = [
...
function Finab() {
    const modalContent = () => {
        const modalContent = document.createDocumentFragment();
        const className = "modal-add-new-prod";
        const formId = "fnb_new_modal";
        const modalInner = document.createElement("div");
        modalInner.classList = className;
        modalInner.innerHTML = `
            <div class="ui-modal__head">
                <h2 class="ui-modal__title">Ajouter un évènement de finab</h2>
            </div>
            <div class="ui-modal__body">
                <form method="post" class="cpn-form" id="" enctype="multipart/form-data" style"display:flex; align-items:center; flex-direction:column;">
                    Bientôt disponible
                </form>
            </div>
        `;
     ...
     const new_event_fnb =document.querySelector('button[data-action="new_event_fnb"]')
    new_event_fnb.addEventListener("click", (e) => {
        e.preventDefault();
        const modal = new MODAL({
            id: "modalNewProduct",
            className: "wdg-modal--default",
